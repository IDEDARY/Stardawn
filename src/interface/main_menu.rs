...
pub fn build_main_menu (commands: &mut Commands, asset_server: &Res<AssetServer>, ui_tree: &mut UiTree) -> Result<(), LunexError> {
    ...
    let background = Widget::create(&mut temporary_tree, &widget.end("background"), layout::Solid::default().with_scaling(SolidScale::Fill).pack())?;

    //Merge the temporary tree to main tree if nothing failed so far
    ui_tree.merge(temporary_tree)?;

    //Make it visible
    background.fetch_mut(ui_tree, "").unwrap().set_visibility(true);

    //Spawn the elements
    commands.spawn(ImageElementBundle::new(background.clone(), &ImageParams::default(), asset_server.load("images/interface/main_menu/background_0.png"), Vec2::new(1920.0,1080.0)));
